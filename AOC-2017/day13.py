...
def main( argv ):
    ...
    print( f"Part 1 answer: { findSeverity( data ) }" )

    ##
    # Part 2
    ##

    # There must be a better way.  Brute force like this takes a bit to produce a result
    # but is correct.

    delay = 0

    while findSeverity( data, delay ) != 0:
        delay += 1

    print( f"Part 2 answer: { delay }" )

def findSeverity( firewall, delay=0 ):
    # If layer = depth + (depth - 2): severity += (depth * layer)
    severity = 0

    for line in firewall:
        depth = int( line.split( ':' )[ 0 ] ) + delay
        range = int( line.split( ':' )[ 1 ] )

        if not (depth) % (range + (range - 2)):
