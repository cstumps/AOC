...
def main( argv ):
    ...
    # Data is just an adjacency list.  Since data is ordered, largest value is last element.  
    # Could use dictionary instead and skip this but list should be faster lookup.
    adjList = [ [] for _ in range( int( data[ -1 ].split()[ 0 ] ) + 1 ) ]

    for line in data:
        line = line.split( '<->' )
        adjList[ int( line[ 0 ] ) ] += list( map( int, line[ 1 ].split( ',' ) ) )

    visited = [-1] * len( adjList )
    visitAllNodes( visited, adjList )

    ##
    # Part 1
    ##

    print( f"Part 1 answer: { visited.count( 0 ) }" )

    ##
    # Part 2
    ##

    print( f"Part 2 answer: { max( visited ) + 1 }" )


# Make sure we visit all nodes once and give each group a unique ID
def visitAllNodes( visited, adj ):
    id = 0

    for i in range( len( adj ) ):
        if visited[ i ] == -1:
            visitNode( i, id, visited, adj )
            id += 1

# Visit a single node and all it's children
def visitNode( node, id, visited, adj ):
    if visited[ node ] == -1:
        visited[ node ] = id

        for c in adj[ node ]:
            visitNode( c, id, visited, adj )
