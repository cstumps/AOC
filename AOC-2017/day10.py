...
def main( argv ):
    ...
    d1 = list( map( int, data.split( ',' ) ) )
    output = doRound( d1 )

    print( f"Part 1 answer: {output[0] * output[1]}" )

    ##
    # Part 2
    ##

    print( f"Part 2 answer: { getHashWithString( data ) }" )


# Re-organized the functions a bit for later puzzles.
def getHashWithString( input ):
    d = list( map( ord, input ) ) + [ 17, 31, 73, 47, 23 ]
    return getHash( d )

def getHash( input ):
    output = doRound( input, rounds=64 )

    # Chunk list into 16 element chunks
    chunks = [ output[ i:i + 16 ] for i in range( 0, len( output ), 16 ) ]

    # Perform XOR on each chunk
    denseHash = [ reduce( lambda x, y: x^y, c ) for c in chunks ]

    # Convert to hash string
    hashStr = ''.join( "%02x" % d for d in denseHash )

    return hashStr

def doRound( lengths, rounds=1 ):
    #string = [ 0, 1, 2, 3, 4 ]
    ...
    for i in range( rounds ):
        for l in lengths:
            # Rotate left by position
            tmp = rotate( string, pos )

            # Slice the string
            slice = (tmp[ :l ])[::-1]

            # Re-assemble and rotate back
            string = rotate( slice + tmp[ l: ], -pos )

            # Increase position and skip
            pos = (pos + l + skip) % len( string )
            skip += 1
