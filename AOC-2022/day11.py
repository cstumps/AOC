...
# This isn't particularily well written and I did require a small hint to realize that
# the key was modulo for part 2.
...
def main( argv ):
    ...
    monkeys = doRounds( monkeys, 20 )
    activity = sorted( [ m.inspections for m in monkeys ], reverse=True )

    print( f"Part 1 answer: {activity[ 0 ] * activity[ 1 ]}" )
    ...
    monkeys = doRounds( monkeys, 10000, modVal )
    activity = sorted( [ m.inspections for m in monkeys ], reverse=True )

    print( f"Part 2 answer: {activity[ 0 ] * activity[ 1 ]}" )

def doRounds( monkeys, count, worry=0 ):
    for i in range( count ):
        for monkey in monkeys:
            monkey.inspections += len( monkey.items )
            monkey.items = eval( 'monkey.items' + monkey.operation )

            if not worry:
                monkey.items = np.floor( monkey.items / 3 )

            else:
                for i in range( len( monkey.items ) ):
                    monkey.items[ i ] %= worry

            for item in monkey.items:
                if item % monkey.test: # False
                    monkeys[ monkey.false ].items = np.append( monkeys[ monkey.false ].items, [item] )
                else: # True
                    monkeys[ monkey.true ].items = np.append( monkeys[ monkey.true ].items, [item] )
...
def parseInput( data ):
    ...
    for line in data:
        ...
        if 'Monkey' in line:
            ...
            curMonkey = Monkey( int( line.split()[ 1 ][ :-1 ] ) )
        elif 'Starting' in line:
            items = line.split( ':' )[ 1 ].split( ',' )
            curMonkey.items = np.array( [ int( i.strip() ) for i in items ]  )
        elif 'Operation' in line:
            obj = line.split()

            if obj[ -1 ] == 'old':
                curMonkey.operation = '**2'
            else:
                curMonkey.operation = obj[ 4 ] + obj[ 5 ]
        elif 'Test' in line:
            curMonkey.test = int( line.split()[ -1 ] )
        elif 'true' in line:
            curMonkey.true = int( line.split()[ -1 ] )
        elif 'false' in line:
            curMonkey.false = int( line.split()[ -1 ] )
    else:
        monkeys.append( curMonkey )
...
class Monkey( object ):
    ...
    def __str__( self ):
        return ( f"Monkey {self.number}:\n"                   \
                 ...
                 f"  Operation: new = old {self.operation}\n"  \
                 f"  Test: divisible by {self.test}\n"        \
                 f"    If true: throw to monkey {self.true}\n" \
                 f"    If false: throw to monkey {self.false}" )
