...
def main( argv ):
    ...
    data = np.asarray( [ list( map( int, line.strip() ) ) for line in data ] )
    ...
    # Only work on interior trees
    for i in range( 1, height - 1 ):
        for j in range( 1, width - 1 ):
            val = data[ j ][ i ]

            upperCol = np.sort( data[ :, i ][:j] )[::-1]
            lowerCol = np.sort( data[ :, i ][j+1:] )[::-1]
            leftRow  = np.sort( data[ j, : ][:i] )[::-1]
            rightRow = np.sort( data[ j, : ][i+1:] )[::-1]

            if (val > upperCol[ 0 ]) or (val > lowerCol[ 0 ]) or (val > leftRow[ 0 ]) or (val > rightRow[ 0 ]):
                visibleTrees += 1

    # Add in border trees
    visibleTrees += ( (width * 2) + ((height - 2) * 2) )

    print( f"Part 1 answer: {visibleTrees}" )
    ...
    for i in range( 1, height - 1 ):
        for j in range( 1, width - 1 ):
            ...
            score = findScore( upperCol, val ) * \
                    findScore( lowerCol, val ) * \
                    findScore( leftRow,  val ) * \
                    findScore( rightRow, val )

            if score > maxScore:
                maxScore = score

    print( f"Part 2 answer: {maxScore}" )

def findScore( arr, val ):
    for y, x in enumerate( arr, 1 ):
        if x >= val:
            return y
    else:
        return len( arr )
