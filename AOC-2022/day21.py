...
# find the grove; they can show you a shortcut if you answer their riddle.
...
def main( argv ):
    ...
    monkeys = { line.split( ':' )[ 0 ]: line.split( ':' )[ 1 ].strip() for line in data } 
    ...
    while len( monkeys.keys() ) > 1:
        delList = []

        for k in monkeys.keys():
            if monkeys[ k ].isdigit():
                for l in monkeys.keys():
                    if k in monkeys[ l ]:
                        monkeys[ l ] = monkeys[ l ].replace( k, monkeys[ k ] )
        ...
        # Collapse any completed expressions
        for key in monkeys.keys():
            try:
                # We have to cast to int here because the above check using isdigit will
                # not intrepret a decimal (eg. 2.0) as a digit.
                monkeys[ key ] = str( int( eval( monkeys[ key ] ) ) )
            except NameError:
                pass
