...
# After doing part one in an ugly fashion, I discoered from a post on SA that the
# next segment position could be figured out from the abs of the distance.  This greatly 
# simplified things.
...
def main( argv ):
    ...
    rope = Rope()
    rope.addKnot( 0, 0 ) # Head
    rope.addKnot( 0, 0 ) # Tail

    for move in data:
        doMove( move[ 0 ], move[ 1 ], rope )

    print( f"Part 1 answer: {len( rope.visited )}" )
    ...
    print( f"Part 2 answer: {len( rope.visited )}" )

def doMove( move, count, rope ):
...
class Rope( object ):
    ...
    def moveHead( self, dx, dy ):
        ...
        for knot in self.knots[ 1: ]:
            dv = np.array( [ prev[ 0 ] - knot[ 0 ], prev[ 1 ] - knot[ 1 ] ] )

            if abs( dv[ 0 ] ) > 1 or abs( dv[ 1 ] ) > 1:
                sdv = np.sign( dv )
        ...
        # Keep track of what we visited
        self.addVisited( prev[ 0 ], prev[ 1 ] )
