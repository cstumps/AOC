...
def main( argv ):
    ...
    # Get the size of the table at the top
    for line in data:
        line = line.strip()

        if line.split()[ 0 ].isnumeric():
            numStacks = int( line[ -1 ].strip() )
            numPadding = 1
    ...
    # Parse out table
    stacks = [ readColumn( data, (i * numPadding) + 1 ) for i in range( numStacks ) ]

    moves = [ [ int( line.strip().split()[ 1 ] ), 
                int( line.strip().split()[ 3 ] ), 
                int( line.strip().split()[ 5 ] ) ] for line in data if 'move' in line ]
    ...
    crates = [ s[:] for s in stacks ]
    moveCrates( crates, moves )
  
    print( "Part 1 answer: %s" % ''.join( [ c[ -1 ] for c in crates ] ) )
    ...
    # Move the crates around
    crates = [ s[:] for s in stacks ]
    moveCrates( crates, moves, False )
   
    print( "Part 2 answer: %s" % ''.join( [ c[ -1 ] for c in crates ] ) )


# Shuffle the crates around.  This modifies the passed in crates.
def moveCrates( crates, moves, reverse=True ):
    for move in moves:
        count = move[ 0 ]
        src   = move[ 1 ] - 1
        dest  = move[ 2 ] - 1
        ...
        if reverse:
            crates[ dest ] += reversed( crates[ src ][ -count: ] )
        else:
            crates[ dest ] += crates[ src ][ -count: ]

        crates[ src ] = crates[ src ][ :-count ]


# Return a given column from the table at the top of the data as a reversed list
def readColumn( data, col ):
    column = []

    for line in data:
        line = line.rstrip().ljust( col + 1 )

        if line[ col ].isnumeric(): # We're done 
            break
