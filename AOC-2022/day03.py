...
def main( argv ):
    ...
    with open( "input/day03-input.txt", "r" ) as f:
        data = [ list( map( getPriority, line.strip() ) ) for line in f ]
    ...
    #data = [ list( map( getPriority, line.strip() ) ) for line in data ]
    ...
    for bag in groups:
        bagA = set( bag )
        bagB = set( next( groups ) )
        bagC = set( next( groups ) )

        score += list( bagA & bagB & bagC )[ 0 ]
