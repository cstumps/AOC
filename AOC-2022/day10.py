...
def main( argv ):
    ...
    #data = [ 'addx 15', 'addx -11', 'addx 6', 'addx -3', 'addx 5', 'addx -1', 'addx -8', 'addx 13', 
    #         'addx 4', 'noop', 'addx -1', 'addx 5', 'addx -1', 'addx 5', 'addx -1', 'addx 5', 
    #         'addx -1', 'addx 5', 'addx -1', 'addx -35', 'addx 1', 'addx 24', 'addx -19', 'addx 1', 
    #         'addx 16', 'addx -11', 'noop', 'noop', 'addx 21', 'addx -15', 'noop', 'noop', 'addx -3', 
    #         'addx 9', 'addx 1', 'addx -3', 'addx 8', 'addx 1', 'addx 5', 'noop', 'noop', 'noop', 
    #         'noop', 'noop', 'addx -36', 'noop', 'addx 1', 'addx 7', 'noop', 'noop', 'noop', 'addx 2', 
    #         'addx 6', 'noop', 'noop', 'noop', 'noop', 'noop', 'addx 1', 'noop', 'noop', 'addx 7', 
    ...
    # Read an easy way to do this after completing mine:
    # replace noops with a "0" and addx V with a "0 V" then the value of X at a given cycle is just
    # the running sum at that position.
    ...
    for i in range( 60, 220+1, 40 ):
        cpu.run( 40 )
        ss += (cpu.x * i)

    print( f"Part 1 answer: {ss}" )
...
class CPU( object ):
    ...
    def run( self, cycles ):
        for i in range( cycles ):
            ...
            # The actual register isn't updated until the third cycle which is part
            # of the next OP code.
            if self.addCycle == 2:
                self.x += self.temp
                self.addCycle = 0
            ...
            if op[ 0 ] == 'n':    # noop
                ...
                # First add cycle
                if not self.addCycle: 
                    self.addCycle = 1

                # Second add cycle
                else: 
                    self.temp = int( op.split()[ 1 ] )
                    self.pc += 1 
                    self.addCycle = 2


    def blit( self ):
        if self.crtCycle in [ self.x-1, self.x, self.x+1 ]:
            self.crtLine += '#'
        else:
            self.crtLine += '.'
