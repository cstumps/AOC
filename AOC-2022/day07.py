...
def main( argv ):
    ...
    # Process the input and build our tree
    for line in data:
        # Our command
        if line[ 0 ] == '$':
            if line[ 1 ] == 'cd':
                ...
                # Change to new sub directory
                    ...
                    ptr = ptr.getChild( line[ 2 ] )

            elif line[ 1 ] == 'ls':
                continue
            else:
                raise MyError( f"Invalid command {line[ 2 ]} found." )

        # Response from system
        elif line[ 0 ] == 'dir':
            n = Node( Node.TYPE_DIR, line[ 1 ] )
            ptr.addChild( n )
            dirs.append( n )

        # Else we have a file with a size
        else:
            ptr.addChild( Node( Node.TYPE_FILE, line[ 1 ], int( line[ 0 ] ) ) )
    ...
    target = 30000000 - (70000000 - root.findSize())
    options = []
    ...
    print( f"Part 2 answer: {options[ 0 ]}" )
...
class Node( object ):
    ...
    def addChild( self, node ):
        for child in self.children: # Duplicate, don't add
    ...
    def getChild( self, childName ):
        for child in self.children:
            ...
            raise MyError( f"Directory {childName} not found.")

    def getParent( self ):
        if self.parent == None:
            raise MyError( f"Attempting to get parent of root node" )
