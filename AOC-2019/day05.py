...
def runProg( mem, i1 ):
    ...
    while True:
        op    = int( str( mem[ pc ] ), 16 ) & 0x000FF
        mode1 = int( str( mem[ pc ] ), 16 ) & 0x00F00
        mode2 = int( str( mem[ pc ] ), 16 ) & 0x0F000
        mode3 = int( str( mem[ pc ] ), 16 ) & 0xF0000
        ...
        if op == 0x01:
            ...
            a = mem[ a ] if not mode1 else a
            b = mem[ b ] if not mode2 else b
        ...
        # Terminate
            ...
            print( "Unknown opcode encountered: pc = %s, op = %s" % (pc, op) )
            break
