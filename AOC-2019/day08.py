...
def main( argv ):
    ...
    # Convert to 1D list of ints
    data = [ int( x ) for x in str( data ) ]

    # Convert to 2D list
    m2 = [ data[ i:i+width] for i in range( 0, len( data ), width ) ]

    # Covert to layers
    layers = [ m2[ i:i+height] for i in range( 0, len( m2 ), height ) ]

    # Count up number of ones and twos on the layer with the least number of 0s
    minZeroCnt = 9999
    minZeroLay = 0
    numOne = 0
    numTwo = 0
    ...
    for i, l in enumerate( layers ):
        flat = [ j for sub in l for j in sub ]
        
        if flat.count( 0 ) < minZeroCnt:
            minZeroCnt = flat.count( 0 )
            minZeroLay = i
            numOne = flat.count( 1 )
            numTwo = flat.count( 2 )

    print( "Part 1: %s" % (numOne * numTwo) )

    # Part 2

    # For each pixel
    numLayers = len( layers )
    picture = [ [ 2 for i in range( width ) ] for j in range( width ) ]
    ...
    for i in range( height ):
        for j in range( width ):
            for l in layers:
                ...
                if pixel != 2:
                    picture[ i ][ j ] = pixel
                    break # Stop processing this pixel, we have an opaque one

    # Now print the picture
    for i in range( height ):
        row = [ str( s ) for s in picture[ i ] ]
        print( "%s" % ' '.join( row ) )
