...
# in Crab), you are quite surprised when the crab starts arranging many cups in
# a circle on your raft - one million (1000000) in total.
...
def main( argv ):
    ...
    for i in range( 100 ):
        ...
        # print( "cups: %s" % printCups( data, curCup ) )

        data, curCup = doMove( data, curCup )
    ...
    print( "" )
    print( "Part 1 answer: %s" % tallyScore( data ) )
    ...
    maxCup = max( input )
    input = input + [ i for i in range( maxCup + 1, 1000001 ) ] # Extend data to 1,000,000

    data = [0] * (len( input ) + 1) # We'll make this based on index 1

    for i in range( len( input ) ):
        data[ input[ i ] ] = input[ (i + 1) % len( input ) ]

    curCup = input[ 0 ]

    for i in range( 10000000 ):
        data, curCup = doMove2( data, curCup )

    star1 = data[ 1 ]
    star2 = data[ star1 ]

    print( "Part 2 answer: %s" % (star1 * star2) )

##
# Functions for part 1 - Kept in for posterity.  Part 2 functions 
# are much faster.
##
def doMove( seq, curCup ):
    # Pick up 3 cups immediately after current cup
    i = seq.index( curCup )
    pick = (seq[ i: ] + seq[ 0:i ])[ 1:4 ]

    #print( "pick up: %s" % ', '.join( [ str( x ) for x in pick ] ) )
    ...
    while destCup not in seq:
        destCup = ((destCup - 1) + hiCup) % hiCup

    #print( "destination: %s" % destCup )

    # Reinsert removed cups after destination cup
    i = seq.index( destCup )
    
    # Return the new sequence and the new current cup
    newSeq = seq[ 0:i+1 ] + pick + seq[ i+1: ]
    newCup = newSeq[ (newSeq.index( curCup ) + 1) % len( newSeq ) ]

    return newSeq, newCup

def printCups( seq, curCup ):
    cups = []
...
def tallyScore( seq ):
    ...
    return ''.join( [ str( x ) for x in (seq[ i+1: ] + seq[ 0:i ]) ] )

##
# Functions for part 2
##
def doMove2( seq, curCup ):
    # Determine which cups to 'remove'
    remLst = [ seq[ curCup ], seq[ seq[ curCup ] ], seq[ seq[ seq[ curCup ] ] ] ]
    ...
    while True:
        ...
        if destCup not in remLst:
            break
        
    # De-couple removed cups
    tmp = seq[ destCup ]

    seq[ curCup ] = seq[ remLst[ 2 ] ] # Point current cup to after cut
    seq[ destCup ] = remLst[ 0 ]        # Point destiation cup to first removed cup
    seq[ remLst[ 2 ] ] = tmp            # Point last removed cup to insertion point
...
def printCups2( seq, curCup, start ):
    cups = []

    while str( start ) not in cups:
        cups.append( str( start ) )
        start = seq[ start ]
