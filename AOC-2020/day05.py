...
def main( argv ):
    ...
    # Read in input file and convert to binary then to decimal (seat ID)
    with open( "input/day05-input.txt", "r" ) as f:
        data = [ int( line.rstrip().translate( table ), 2 ) for line in f ]

    # Part 1

    print( "Part 1 answer: %s" % max( data ) )

    # Part 2
    data.sort()
    
    # Since we sorted it we dont' have to worry about the edge cases assuming
    # there is exactly one missing seat.
    for i, seat in enumerate( data ):
        if data[ i + 1 ] != seat + 1: # Seat missing
            break
    
    print( "Part 2 answer: %s" % (seat + 1) )
