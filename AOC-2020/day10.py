...
def main( argv ):
    ...
    with open( "input/day10-input.txt", "r" ) as f:
        data = [ 0 ] + [ int( line.rstrip() ) for line in f ]

    #data = [ 0 ] + [ 16, 10, 15, 5, 1, 11, 7, 19, 6, 12, 4 ]
    #data = [ 0 ] + [ 28, 33, 18, 42, 31, 14, 46, 20, 48, 47, 24, 23, 49, 45, 19, 38, 39, 11, 1, 32, 25, 35, 8, 17, 7, 9, 4, 2, 34, 10, 3 ]

    # Part 1

    data.sort()
    diff = [ data[ i ] - data[ i-1 ] for i in range( 1, len( data ) ) ]
    ans = diff.count( 1 ) * (diff.count( 3 ) + 1)  # Our device is always 3 jolts higher than highest adapter.

    print( "Part 1 answer: %s" % ans )

    # Part 2

    lookup = [ 1, 1, 2, 4, 7 ]  # Longest string of 1's in diff data is 4
    count = 0
    ans = 1
    ...
    ans *= lookup[ count ]  # If we end on a 1, record it too
