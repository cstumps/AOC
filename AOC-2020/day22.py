...
def main( argv ):
    ...
    # Read in input file
    with open( "input/day22-input.txt", "r" ) as f:
    ...
    # data = [ 'Player 1:', # Part 1 doesn't like these decks
    #          '43',
    #          '19',
    #          '',
    #          'Player 2:',
    ...
    # As long as someone still has cards
    while len( pDeck ) and len( cDeck ):
        # Take the top card off each and compare
        pCard = pDeck.pop()
        cCard = cDeck.pop()

        if pCard > cCard:
            pDeck = [ cCard, pCard ] + pDeck
        else:
            cDeck = [ pCard, cCard ] + cDeck
        
    # If we got here, one of the decks is empty
    if not len( pDeck ):
        score = tallyScore( cDeck )
    else:
        score = tallyScore( pDeck )
    ...
    winner, deck = playGame( pDeck, cDeck ) 
    
    score = tallyScore( deck )

    print( "Part 2 answer: %s" % score )

def playGame( pDeck, cDeck ):
    deckCache = []

    while len( pDeck ) and len( cDeck ):
        # Check here for infinite recursion
        key = ''.join( [ str( x ) for x in pDeck + [':'] + cDeck ] ) # A separator is needed here.  If we just cat 
                                                                     # the two decks it's possible that the sequence 
                                                                     # is non-unique.

        if key in deckCache:
            return 1, pDeck 
        else:
            deckCache.append( key )

        # Otherwise play the round normally
        winner, wCard, lCard = playRound( pDeck, cDeck )

        # Update decks based on the winner
        if winner == 1:
            pDeck = [ lCard, wCard ] + pDeck
        else:
            cDeck = [ lCard, wCard ] + cDeck
...
# Returns: winner, winning card, losing card
def playRound( pDeck, cDeck ):
    # Grab the top card of each deck
    pCard = pDeck.pop()
    cCard = cDeck.pop()

    winner = 0

    # Round determined by recursive combat
    if (len( pDeck ) >= pCard) and (len( cDeck ) >= cCard):
        winner, _ = playGame( pDeck.copy()[ -pCard: ], cDeck.copy()[ -cCard: ] )
        
    # Otherwise, round determined by normal combat
    if ((pCard > cCard) and winner == 0) or (winner == 1):
        return 1, pCard, cCard

    else:
        return 2, cCard, pCard

# Compute the score of a deck.  Expects deck to be in reverse order.
def tallyScore( deck ):
    score = 0
