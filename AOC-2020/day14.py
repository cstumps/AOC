...
# I suspect that this is far from an optimal solution.  While the generateNumbers
# function works to find the bit patterns we care about it seems sloppy.  Not sure
# how to improve it.
...
def main( argv ):
    ...
    for line in data:
        lineVal = line.split()[ 2 ]

        if 'mask' in line:
            addMask = int( lineVal.replace( 'X', '0' ), 2 )
            subMask = int( lineVal.replace( 'X', '1' ), 2 )
        else:
            val = (int( lineVal ) | addMask) & subMask
            mem[ int( line.split()[ 0 ][ 4:-1 ] ) ] = val

    print( "Part 1 answer: %s" % sum( mem.values() ) )
    ...
    for line in data:
        ...
        if 'mask' in line:
            pos = [ len( lineVal ) - i - 1 for i in range( len( lineVal ) ) if lineVal[ i ] == 'X' ]
            addMask = int( lineVal.replace( 'X', '0' ), 2 )
            subMask = int( lineVal.replace( '1', '0' ).replace( 'X', '1' ), 2 )
        
        else:
            loc = (int( line.split()[ 0 ][ 4:-1 ] ) | addMask) & ~subMask

            for m in generateNumbers( pos, loc ):
                mem[ m ] = int( lineVal )

    print( "Part 2 answer: %s" % sum( mem.values() ) )

# Take a list of bit positions and generate all numbers from them
def generateNumbers( bits, mask ):
    totalNums = 2**len( bits )
    nums = []#[ 0, mask ]

    for i in bits:
        nums.append( (1 << i) | mask )
        nums.append( (0 << i) | mask )

    comb = combinations( nums, len( bits ) )
    nums = [ reduce( ior, i ) for i in comb ]
...
# This guy did something similar but had a much better and faster way to
# generate the sequences based on 'X':
