...
def main( argv ):
    ...
    for line in data:
        ...
        if not len( line.strip() ):
            # Remove duplicates and add to list of counts
            counts.append( len( set( questions ) ) )
            counts2.append( len( set.intersection( *questions2 ) ) )
            
            questions.clear()
            questions2.clear()

        else:
            # Process this persons responses
            questions += list( line.strip() )
            questions2.append( set( list( line.strip() ) ) )

    # Process last group
    counts.append( len( set( questions ) ) )
    counts2.append( len( set.intersection( *questions2 ) ) )

    print( "Part 1 answer: %s" % sum( counts ) )
    print( "Part 2 answer: %s" % sum( counts2 ) )
