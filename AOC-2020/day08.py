...
def main( argv ):
    ...
    #   'Infinite loop detected: acc = 1749, lastJmp = 316'
    #
    # 1749 is the answer to part 1.  For part 2, change line 316 to a nop:
    ...
    # First document the jmps
    for i, line in enumerate( data ):
        if 'jmp' in line:
            jumps.append( i )

    # Now, for each jump try and no-op it and see if the program terminates
    for j in jumps:
        oldOp = data[ j ]
        data = data[ 0:j ] + ['nop'] + data[ j+1: ]
        ...
        try:
            ...
            # Not this one! 
            mach.reset()
            data = data[ 0:j ] + [oldOp] + data[ j+1: ] # Restore the jmp
        else:
            break # yay, we finished!
...
class Interp( object ):
    def __init__( self ):
        self.acc = 0
        self.pc  = 0
        self.trace = []  # Line numbers we've executed
    ...
    # Run program and halt on infinite loop
    def runProg( self, mem ):
        while True:
            # See if we're in an infinite loop
            if self.pc in self.trace:
                raise MyError( "Infinite loop detected: acc = %s" % self.acc )
            else:
                self.trace.append( self.pc ) # Log which instruction we're visiting

            op = mem[ self.pc ].split()[ 0 ]
            
            if op == 'acc':
                self.acc += int( mem[ self.pc ].split()[ 1 ] )
                self.pc += 1

            elif op == 'jmp':
                self.pc += int( mem[ self.pc ].split()[ 1 ] )
            ...
            # Program end?
            if self.pc == len( mem ):
                print( "Program terminated: acc = %s" % self.acc )
                return
