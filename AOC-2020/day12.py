...
# Migth have been better to perhaps pass in if a waypoint was needed at class
# init rather than direcly in the functions.  Not a big deal though.
...
def main( argv ):
    ...
    for action in data:
        if action[ 0 ] in [ 'N', 'S', 'E', 'W' ]:
            ship.move( int( action[ 1: ] ), action[ 0 ] )
        elif action[ 0 ] == 'F':
            ship.move( int( action[ 1: ] ) )
        elif action[ 0 ] == 'L':
            ship.turn( -int( action[ 1: ] ) )
        elif action[ 0 ] == 'R':
            ship.turn( int( action[ 1: ] ) )

    print( "Part 1 answer: %s" % ship.manhattan() )
    ...
    for action in data:
        if action[ 0 ] in [ 'N', 'S', 'E', 'W' ]:
            ...
            ship.move( int( action[ 1: ] ), waypoint=True )
        elif action[ 0 ] == 'L':
            ship.turn( -int( action[ 1: ] ), waypoint=True )
        elif action[ 0 ] == 'R':
            ship.turn( int( action[ 1: ] ), waypoint=True )

    print( "Part 2 answer: %s" % ship.manhattan( waypoint=True ) )
...
class Ship( object ):
    ...
    def __init__( self, waypoint=False ):
        self.bearing = 90.0 # Direction we're facing, starts east.

        # Position
        self.x = 0
        self.y = 0

        # Waypoint based positioning (part 2)
        self.actualX = 0
        self.actualY = 0
    ...
    def move( self, amt, dir=None, waypoint=False ):
        if dir == 'N':
            self.y -= amt
        elif dir == 'S':
            self.y += amt
            ...
            if not waypoint:
                self.y += (amt * round( -math.cos( math.radians( self.bearing ) ) ))
                self.x += (amt * round( math.sin( math.radians( self.bearing ) ) ))
            else:
                self.actualY += (amt * self.y)
                self.actualX += (amt * self.x)

    def turn( self, deg, waypoint=False ):
        self.bearing = ( (deg + self.bearing) % 360 )

        if waypoint:
            newX = round( (self.x * math.cos( math.radians( deg ) )) - (self.y * math.sin( math.radians( deg ) )) )
            newY = round( (self.y * math.cos( math.radians( deg ) )) + (self.x * math.sin( math.radians( deg ) )) )

            self.x = newX
            self.y = newY

    def manhattan( self, waypoint=False ):
        if not waypoint:
            return math.fabs( float( self.x ) ) + math.fabs( float( self.y ) )
        else:
            return math.fabs( float( self.actualX ) ) + math.fabs( float( self.actualY ) )
