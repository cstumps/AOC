...
# My answer to part one is pretty elegant and fast.  Part 2 however is ugly and a bit 
# slow.  There should be a much more intelligent way to do this but I'm already behind
# on the puzzles so dont' want to dwell on it.

import sys 
import numpy as np
from scipy.signal import convolve2d

# Convert symbols to numbers and vice versa
mapInt  = lambda x : 0 if x == 'L' else (1 if x == '#' else 16)
mapChar = lambda x : 'L' if x == 0 else ('#' if x == 1 else '.')
...
def main( argv ):
    ...
    oldBoard = []
    board = np.asarray( [ list( map( mapInt, line ) ) for line in data ] )

    while not np.array_equal( oldBoard, board ):
        oldBoard = board
        board = stepBoard( board )

    print( "Part 1 answer: %s" % np.count_nonzero( board == 1 ) )

    # Part 2

    # Convert all the seats to points (more readable than a comprehension)
    oldBoard = []
    board = np.asarray( [ list( line ) for line in data ] )

    while not np.array_equal( oldBoard, board ):
        oldBoard = board.copy()
        board = stepBoard2( board )

    print( "Part 2 answer: %s" % np.count_nonzero( board == '#' ) )

# Returns new board of counts seen by each location
def stepBoard2( b ):
    counts = np.zeros( (len( b ), len( b[ 0 ] )) )

    # Iterate through the board
    for i in range( len( b ) ):
        for j in range( len( b[ i ] ) ):
            # Skip locations without a chair
            if b[ i ][ j ] == '.':
                continue
            ...
            # Up right
            for k, l in zip( range( i-1, -1, -1 ), range( j+1, len( b[ i ] ) ) ):
                if b[ k ][ l ] != '.':
                    if b[ k ][ l ] == '#':
                        counts[ i ][ j ] += 1
                    break
            ...
            # Down right
            for k, l in zip( range( i+1, len( b ) ), range( j+1, len( b[ i ] ) ) ):
                if b[ k ][ l ] != '.':
                    if b[ k ][ l ] == '#':
                        counts[ i ][ j ] += 1
                    break
            ...
            # Down left
            for k, l in zip( range( i+1, len( b ) ), range( j-1, -1, -1 ) ):
                if b[ k ][ l ] != '.':
                    if b[ k ][ l ] == '#':
                        counts[ i ][ j ] += 1
                    break
    ...
    # Go through again and update the board
    for i in range( len( b ) ):
        for j in range( len( b[ i ] ) ):
            # Skip locations without a chair
            if b[ i ][ j ] == '.':
...
def stepBoard( b ):
    ...
    emptyMask = (b == 16).astype( int ) << 4

    count = convolve2d( b, kernel, mode='same' ) & 15

    return ((b == 0) & (count == 0)) | ( ((b == 1) & (count < 4)) & b ) | emptyMask
