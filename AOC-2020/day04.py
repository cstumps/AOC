...
def main( argv ):
    ...
    # Process the data
    for line in data:
        # Blank line indicates new passport start
        if not len( line.strip() ):
            curPass = Passport()
            passports.append( curPass )

        # Otherwise send the line off for processing
        curPass.addData( line.strip() )

    # Work out how many are valid
    validCount  = 0
    validCount2 = 0
    ...
    print( "Part 1 answer: %s" % validCount )
    print( "Part 2 answer: %s" % validCount2 )
...
class Passport( object ):
    ...
    def isValid( self, validate=False ):
        if not validate:
        ...
        # Perform validation
            ...
            if (not self.byr or not self.iyr or not self.eyr or 
                not self.hgt or not self.hcl or not self.ecl or not self.pid):
                return False

            v1 = self.validateDate( self.byr, 1920, 2002 )
            v2 = self.validateDate( self.iyr, 2010, 2020 )
            v3 = self.validateDate( self.eyr, 2020, 2030 )
            v4 = self.validateHeight( self.hgt )
            v5 = self.validateHair( self.hcl )
            v6 = self.validateEye( self.ecl )
            v7 = self.validatePass( self.pid )

            return v1 and v2 and v3 and v4 and v5 and v6 and v7 

    def validateDate( self, str, min, max ):
        try:
            if str and int( str ) >= min and int( str ) <= max:
                return True
            else:
                return False
        except ValueError:
    ...
    def validateHeight( self, str ):
        m = re.search( '^(\d+)(cm|in)$', str )
        ...
        if m and len( m.groups() ) == 2:
            ...
            if int( m.group(1) ) >= min and int( m.group(1) ) <= max:
                return True
            
        return False

    def validateHair( self, str ):
        m = re.match( '^#[0-9a-fA-F]{6}$', str )
