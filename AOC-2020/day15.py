...
# Your puzzle input is 6,4,12,1,20,0,16.
...
def main( argv ):
    ...
    lastSeen = { 6:1, 4:2, 12:3, 1:4, 20:5, 0:6, 16:7 }
 
    print( "Part 1 answer: %s" % runGame( lastSeen.copy(), 2020 ) )
    print( "Part 2 answer: %s" % runGame( lastSeen.copy(), 30000000 ) )

def runGame( seed, endTurn ):
    lastVal = 0
    
    for i in range( len( seed.keys() ) + 1, endTurn ):
        try:
            newVal = (i - seed[ lastVal ])
            seed[ lastVal ] = i
            lastVal = newVal
        except:
            seed[ lastVal ] = i
