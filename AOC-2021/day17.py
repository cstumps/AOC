...
def main( argv ):
    ...
    # Clean up input values, should be read from file

    # This is another terrible solution.  Certainly not the clever way to do it.  Additionally
    # I am gettig duplicate values in the max arrays.  That's why the conversion to set to remove
    # those.  I simply don't have time to dive in and explore if I want to stay up to date on the
    # puzzles.
    ...
    #validX = runHorizontal( minX, maxX )
    validY = runVertical( minY, maxY )
    validX = runHorizontal2( validY, minX, maxX )
    
    maxVals = []

    #for vx, t1 in validX:
    #    for vy, t2, m in validY:
    #        if (t1 == t2) or (t2 >= t1 and t1 == -1):
    #            print( "vx = %s, vy = %s, max = %s" % (vx, vy, m) )
    #            #maxVals.append( m )
                
    #print( "vx = %s, vy = %s, max = %s" % (vx, vy, m) )
    for vx, vy, maxH in validX:
        maxVals.append( maxH )

    print( "Part 1 answer: %s" % max( maxVals ) )
    print( "Part 2 answer: %s" % len( maxVals ) )


def runVertical( minY, maxY ):
    maxValues = []

    # For each valid X timestamp, figure out if Y also falls in the target zone
    for v in range( -999, 999 ):  # For each velocity
        currY = 0
        currV = v
        maxH = 0
        t = 0
        ...
        while currY >= minY:
            ...
            if currY >= minY and currY <= maxY:
                maxValues.append( (v, t, maxH) )

    return set( maxValues )

def runHorizontal2( validY, minX, maxX ):
    validX = []

    for v in range( 1, maxX + 1 ):
        for vy, targetT, maxH in validY:   
            currV = v
            currX = 0 
            t = 0
            ...
            if currX >= minX and currX <= maxX:
                validX.append( (v, vy, maxH) )
                

    return set( validX )


def runHorizontal( minX, maxX ):
    validV = []

    # If initial velocity were ever at maxX, we'd just skip over the target in one step
    for v in range( 1, maxX + 1 ):  # For each possible starting horizontal velocity
        currV = v
        currX = 0
        t = 0
        ...
        while currV >= 0:
            ...
            if currX >= minX and currX <= maxX:
                if currV == -1:
                    validV.append( (v, -1) ) # -1 indicates that we're in free fall with no horizontal V
                else:
                    validV.append( (v, t) )
