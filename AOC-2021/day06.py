...
def main( argv ):
    ...
    # Init our groups
    groups = [ LanternGroup( i ) for i in range( 9 ) ]

    ##
    # Part 1
    ##

    # Config groups based on input
    for i in data.split( ',' ):
        groups[ int(i) ].count += 1

    count = runDays( groups, 80 )
    ...
    count = runDays( groups, 256 )
    print( "Part 2 answer: %s" % count )


def runDays( groups, days ):
    for day in range( days ):
        # Figure out which updates to make
        for g in groups:
            idx = (g.life - 1) % 9
            groups[ idx ].pending += g.count

            if g.life == 0:
                groups[ 6 ].pending += g.count
...
class LanternGroup( object ):
    def __init__( self, life, count=0 ):
        self.life = life
        self.count = count
        self.pending = 0
