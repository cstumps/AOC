...
def main( argv ):
    ...
    board = np.asarray( [ list( map( int, line.strip() ) ) for line in data ] )
    ...
    while True:
        ...
        # Reset the list of octopuses that have already flashed
        lastFlashed = np.zeros( board.shape, dtype=bool )

        while True:
            # See who flashed (who is greater than 9)
            totalFlashed = board > 9

            # If no one new has flashed, we're done with this step
            if np.array_equal( totalFlashed, lastFlashed ):
                break

            # Look at who has already flashed in previous iterations and dont' re-flash them
            flashed = totalFlashed & ~lastFlashed
            lastFlashed = totalFlashed

            # See who has their energy raised due to a neighbor flashing and by how much
            count = convolve2d( flashed, kernel, mode='same' )

            # Add in the new counts
            board += count

        # Zero out anyone who flashed during this step
        board = np.multiply( board, np.invert( totalFlashed ).astype( int ) )

        flashCount += np.count_nonzero( totalFlashed )
        i += 1

        if ( i == 100 ):  # We're making the assumption that the sync step is beyond 100
            print( "Part 1 answer: %s" % flashCount )

        if (np.count_nonzero( board==0 ) == board.size):
            print( "Part 2 answer: %s" % i )
            break
...
##
# Someone else who also used convolution had a little more compact answer shown here
# Would be worth understanding the numpy calls that I'm not familiar with here.
##
