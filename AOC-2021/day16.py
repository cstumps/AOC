...
def main( argv ):
    ...
    with open( "input/day16-input.txt", "r" ) as f:
        byteStr = f.readline().strip()

    #p = Packet( 'D2FE28' )
    #byteStr = 'A0016C880162017C3686B18A3D4780'
    #byteStr = '9C0141080250320F1802104A08'
    bitStr = bin( int( byteStr, 16 ) )[2:].zfill( len( byteStr ) * 4 )
    ...
    #print( p )
    print( p.packetScore() )
    print( p.computePacket() )

class Packet( object ):
    def __init__( self, bitStr ):
        # Header
        self.version = int( bitStr[ 0:3 ], 2 )
        self.typeID  = int( bitStr[ 3:6 ], 2 )

        # Packet type
        if self.typeID == 4:  # Literal
            bitStr = self.processLiterals( bitStr[ 6: ] )

        else:  # Operator
            bitStr = self.processPackets( bitStr[ 6: ] )

    def computePacket( self ):
        if self.typeID == 0:  # Sum packet
            value = 0

            for i in self.data:
                value += i.computePacket()

        elif self.typeID == 1:  # Product packet
            value = 1

            for i in self.data:
                value *= i.computePacket()

        elif self.typeID == 2:  # Minimum packet
            v = [ i.computePacket() for i in self.data ]
            value = min( v )

        elif self.typeID == 3:  # Maximum packet
            v = [ i.computePacket() for i in self.data ]
            value = max( v )

        elif self.typeID == 4:  # Value packet
            for i in self.data:
                value = int( ''.join( self.data ), 2 )

        elif self.typeID == 5:  # Greater than
            value = int(self.data[ 0 ].computePacket() > self.data[ 1 ].computePacket())

        elif self.typeID == 6:  # Less than
            value = int(self.data[ 0 ].computePacket() < self.data[ 1 ].computePacket())

        elif self.typeID == 7:  # Equal
            value = int(self.data[ 0 ].computePacket() == self.data[ 1 ].computePacket())

        return value

    def packetScore( self ):
        score = 0
        ...
        return self.version + score

    def packetLength( self ):
        if self.typeID == 4:
            return (len( self.data ) * 5) + 6
        else:
            if self.mode:
                length = 18
            else:
        ...
        # I think this function is where we're failing because it's not calculating the lengths right
        # We are probably not adjusting bitStr correctly coming back from a recursive call

    def processLiterals( self, bitStr ):
        self.data = []

        for i in range( 0, len( bitStr ), 5 ):
            self.data.append( bitStr[ i+1:i+5 ] )
    ...
    def processPackets( self, bitStr ):
        self.mode = int( bitStr[ 0 ], 2 )
        self.data = []

        if self.mode:  # Mode 1
            self.pktData = int( bitStr[ 1:12 ], 2 )
            bitStr = bitStr[ 12: ]

            for i in range( self.pktData ):
                p = Packet( bitStr )
                bitStr = bitStr[ p.packetLength(): ]
                self.data.append( p )

            return bitStr

        else:
            self.pktData = int( bitStr[ 1:16 ], 2 )

            # Read sub packets
            count = 0
            bitStr = bitStr[ 16: ]

            while count < self.pktData:
                p = Packet( bitStr[ count: ] )
                count += p.packetLength()
                self.data.append( p )
    ...
    def __str__( self ):
        stub = "Version = %s, Type ID = %s" % (self.version, self.typeID) 
