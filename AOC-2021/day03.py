...
#     11001. Then, consider the second bit of the 7 remaining numbers: there are
#     more 0 bits (4) than 1 bits (3), so keep only the 4 numbers with a 0 in
#     the second position: 10110, 10111, 10101, and 10000. In the third
...
# Not particlarily proud of this solution.  It works but I'm sure there is a 
# cleaner way.
...
def main( argv ):
    ...
    for i in range( width ):
        # Mask off a bit
        d = [ (j & 2**i) >> i for j in data ] 

        # If 1 occurs most ofter flag it in the power level
        if sum( d ) > height / 2:
            gamma |= (1 << i)

    # Get the epsilon value
    epsilon = ~gamma & ( (2**width) - 1 )

    print( "Part 1 answer: %s" % (gamma * epsilon) )

    ##
    # Part 2
    ##

    O2 = findRating( data, width )
    CO2 = findRating( data, width, True )

    print( "Part 2 answer: %s" % (O2 * CO2) )

def findRating( data, width, inv=False ):
    for i in reversed( range( width ) ):
        common, tie = findCommon( data, i, int( inv == False ) )

        if inv and not tie:
            common ^= 1

        # Select entries which match most/least common value
        d = [ j for j in data if ((j >> i) & 1) == common ]

        if len( d ) == 1:
            return d[ 0 ]

        data = d # Start with what we had left

def findCommon( data, pos, tie ):
    # Mask off the desired bit position
    d = [ (j & 2**pos) >> pos for j in data ]
    s = sum( d )
