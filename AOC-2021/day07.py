...
def main( argv ):
    ...
    # Shortest travel distance ends up just being the median:
    # https://stackoverflow.com/questions/7154978/shortest-distance-travel-common-meeting-point

    med = statistics.median( data )
    fuel = [ abs( i - med ) for i in data ]

    print( "Part 1 answer: %s" % sum( fuel ) )
    ...
    # This is also not the fastest way to find this.  Like part 1 there should be 
    # a calculation that can be done to arrive at the correct answer rather than 
    # brute forcing is as done here.
    for pos in range( minPos, maxPos + 1 ):
        ...
        if fuelSpent < bestFuel or bestFuel == 0:
            bestFuel = fuelSpent
    
    print( "Part 2 answer: %s" % bestFuel )
