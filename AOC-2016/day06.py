...
def main( argv ):
    ...
    print( f"Part 1 answer: {findCode( data, mostLikely=True )}" )

    ##
    # Part 2
    ##

    print( f"Part 2 answer: {findCode( data )}" )


def findCode( messages, mostLikely=False ):
    msg = ''

    for col in range( len( messages[ 0 ] ) ):
        letters = [ d[ col ] for d in messages ]
        freq = { i: letters.count( i ) for i in set( letters ) }
        keys = dict( sorted( freq.items(), key=lambda item: item[1], reverse=mostLikely ) ).keys()
        msg += list( keys )[ 0 ]
