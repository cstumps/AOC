...
def main( argv ):
    ...
    with open( "input/day04-input.txt", "r" ) as f:
        data = [ re.findall( r'(\w+)+', l ) for l in f.readlines() ]
    ...
    for room in data:
        if validateRoom( room ):
            ...
            sectorSum += int( room[ -2 ] )

    #validateRoom( [ 'not', 'a', 'real', 'room', '404', 'oarel' ] )

    print( f"Part 1 answer: {sectorSum}" )
    ...
    print( f"Part 2 answer: {objStorage}" )


def validateRoom( room ):
    # Cat all room name parts
    s = ''.join( room[ :-2 ] )

    # Find a sort character frequencies
    freq = { i: s.count( i ) for i in set( s ) }
    sortedFreq = dict( sorted( freq.items(), key=lambda item: (-item[1], item[0]) ) )

    # Make sure each letter in the checksum is in the top five highest counts
    for c in room[ -1 ]:
        if c not in list( sortedFreq.keys() )[ :5 ]:
            return False
...
def shiftLetter( letter, shift ):
    ...
    idx = (idx + shift) % len( string.ascii_lowercase )
