...
def main( argv ):
    ...
    print( f"Part 1 answer: {len( lookAndSay( data, 40 ) )}" )

    ##
    # Part 2
    ##

    print( f"Part 2 answer: {len( lookAndSay( data, 50 ) )}" )


def lookAndSay( data, count ):
    for _ in range( count ):
        seq = [ m.group(0) for m in re.finditer( r"(\d)\1*", data) ]
        data = ''.join( [ (str( len( entry) ) + entry[ 0 ]) for entry in seq ] )
