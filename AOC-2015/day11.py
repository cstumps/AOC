...
def main( argv ):
    ...
    data = nextValidPass( data )

    print( f"Part 1 answer: {data}" )
    ...
    print( f"Part 2 answer: {data}" )

# Pretty inelegant implementation but it works.
...
def checkRules( s ):
    straights = [ 'abc', 'bcd', 'cde', 'def', 'efg', 'fgh', 'pqr', 'qrs', 'rst', 'stu', 
                 'tuv', 'uvw', 'vwx', 'wxy', 'xyz' ]
    ...
    chk = set( [ m.group() for m in re.finditer( r'((\w)\2)+', s ) ] )

    overlap = 0
    nonOverlap = 0

    for item in chk:
        if len( item ) > 2: # overlapping pair
            overlap += 1
        else:
            nonOverlap += 1
