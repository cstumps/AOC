...
def main( argv ):
    ...
    for line in data:
        raw += len( line.strip() )
        chars += len( eval( f'str({line.strip()})' ) )

    print( f"Part 1 answer: {raw - chars}" )

    ##
    # Part 2
    ##

    # I admit to getting the 2x replacement off the reddit feed
    ...
    for line in data:
        raw += len( line.strip() )
        escaped += len( line.strip().replace( '\\', '\\\\' ).replace( '"', '\\"' ) ) + 2

    print( f"Part 2 answer: {escaped - raw}" )
