...
def main( argv ):
    ...
    ingredients = np.array( [ [  3, 0,  0, -3, 2 ],
                              [ -3, 3,  0,  0, 9 ],
                              [ -1, 0,  4,  0, 1 ],
                              [  0, 0, -2,  2, 8 ] ] )

    #ingredients = np.array( [ [ -1, -2, 6, 3, 8 ], 
    #                          [ 2, 3, -2, -1, 3 ] ] )
        ...
        for j in range( 0, 101 - i ):
            for k in range( 0, 101 - i - j ):
                for l in range( 0, 101 - i - j - k ):
                    score, _ = computeScore( ingredients, [ i, j, k, l ] )

                    if score > bestScore:
                        bestScore = score

    print( f"Part 1 answer: {bestScore}" )
    ...
    for i in range( 0, 101 ):
        for j in range( 0, 101 - i ):
            for k in range( 0, 101 - i - j ):
                for l in range( 0, 101 - i - j - k ):
                    score, calories = computeScore( ingredients, [ i, j, k, l ] )

                    if calories == 500 and score > bestScore:
                        bestScore = score

    print( f"Part 2 answer: {bestScore}" )

def computeScore( ingredients, counts ):
    score = 1

    totals = ingredients[:,:-1 ].T * counts
    calories = ingredients[ :,-1: ].T * counts
