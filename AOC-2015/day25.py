...
def main( argv ):
    ...
    with open( "input/day25-input.txt", "r" ) as f:
        row, col = re.findall( r'\d+', f.readline() )

    row = int( row )
    col = int( col )

    size = max( row, col ) * 2 + 1
    triangle = floydTriangle( size )

    ##
    # Part 1
    ##

    print( f"Part 1 answer: {int( triangle[ row - 1, col - 1 ] )}" )
    

def floydTriangle( size ):
