...
def main( argv ):
    ...
    for line in data:
        ...
        deer.append( Deer( line[ 0 ], int( line[ 3 ] ), int( line[ 6 ] ), int( line[ -2 ] ) ) )

    ##
    # Part 1
    ##

    distances = [ d.computeDistance( 2503 ) for d in deer ]

    print( f"Part 1 answer: {sorted( distances )[ -1 ]}" )
    ...
    for _ in range( 2503 ):
        # Increment each deer 1 second
        for d in deer:
            d.tick()

        # Figure out who gets the point
        maxDis = 0
        winner = ''

        for d in deer:
            if d.distance > maxDis:
                maxDis = d.distance
                winner = d.name

        points[ winner ] += 1

    print( f"Part 2 answer: {sorted( points.values() )[ -1 ]}" )
...
class Deer:
    ...
    def reset( self ):
        self.running = True  # Start off running
        self.distance = 0
        self.timer = self.runTime

    def computeDistance( self, time ):
        distance = 0
        totalTime = self.runTime + self.restTime

        # Find whole period they got thru
        runs = math.floor( time / totalTime )
        distance += runs * (self.speed * self.runTime)

        # Now find how far they went on the last partial period
        partial = time % totalTime
        distance += min( self.runTime, partial ) * self.speed
