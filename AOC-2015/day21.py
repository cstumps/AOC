...
def main( argv ):
    ...
    bossStats = dict( zip( keys, data ) )
    playerStats = dict( zip( keys, [ 100, 0, 0 ] ) )
    ...
    store = { 'weapons': [ [ 8,   [ 4, 0 ] ],
              ...
              'rings':   [ [ 25,  [ 1, 0 ] ],
                           ...
                           [ 40,  [ 0, 2 ] ],
                           [ 80,  [ 0, 3 ] ],
                           [ 0,   [ 0, 0 ] ] ] }  # Rings are optional

    lowestCost = 1000000
    greatestCost = 0

    # For each pair of rings, weapon, and armor
    for i in [ 1, 2 ]:
        for rings in it.permutations( store[ 'rings' ], i ):
            for weapon in store[ 'weapons' ]:
                for armor in store[ 'armor' ]:
                    playerStats[ 'dmg' ] = weapon[ 1 ][ 0 ] + rings[ 0 ][ 1 ][ 0 ]
                    playerStats[ 'armor' ] = armor[ 1 ][ 1 ] + rings[ 0 ][ 1 ][ 1 ]
                    cost = weapon[ 0 ] + armor[ 0 ] + rings[ 0 ][ 0 ]

                    if len( rings ) > 1:
                        playerStats[ 'dmg' ] += rings[ 1 ][ 1 ][ 0 ]
                        playerStats[ 'armor' ] += rings[ 1 ][ 1 ][ 1 ]
                        cost += rings[ 1 ][ 0 ]

                    if playerWin( playerStats, bossStats ):
                        lowestCost = min( cost, lowestCost )
                    else:
                        greatestCost = max(cost, greatestCost )

    ##
    # Part 1
    ##
                    
    print( f"Part 1 answer: {lowestCost}" )

    ##
    # Part 2
    ##

    print( f"Part 2 answer: {greatestCost}" )


def playerWin( player, boss ):
    bossLoss   = max( player[ 'dmg' ] - boss[ 'armor' ], 1 )
    playerLoss = max( boss[ 'dmg' ] - player[ 'armor' ], 1 )

    return (boss[ 'hp' ] / bossLoss) <= (player[ 'hp' ] / playerLoss)
