...
# The key to this puzzle is to realize that you can find the target weight and
# that you only need to find the first group, not all three.
...
def main( argv ):
    ...
    with open( "input/day24-input.txt", "r" ) as f:
        data = np.array( sorted( [ int( line.strip() ) for line in f ], reverse=True ) )

    ##
    # Part 1
    ##

    print( f"Part 1 answer: {findSmallestQuantum( data, data.sum() / 3 )}" )

    ##
    # Part 2
    ##

    print( f"Part 2 answer: {findSmallestQuantum( data, data.sum() / 4 )}" )


def findSmallestQuantum( packages, weight ):
    foundSmallestLength = False
    quantums = []

    for i in range( 1, len( packages ) - 2 ):
        for c in it.combinations( packages, i ):
            c = np.array( c )

            if c.sum() == weight:
                quantums.append( c.prod() )
                foundSmallestLength = True

        if foundSmallestLength:
            break

    return sorted( quantums )[ 0 ]
