...
def main( argv ):
    ...
    directions = [ ['L', 'R'].index( c ) for c in data[ 0 ] ]
    nodes = { l[ 0:3 ]: (l[ 7:10 ], l[ 12:15 ]) for l in data[ 2: ] }
    ...
    print( f"Part 1 answer: {moves}" )

    ##
    # Part 2
    ##

    # For each start node, find out how many moves it takes to get to it's first end
    # state.  These states are cyclic, therefore the least common multiple of the move
    # count will be the number of moves it takes for them all to line up at the end
    # state at the same time.

    start = [ n for n in nodes.keys() if n[ 2 ] == 'A' ]
    moves = [ findEnd( nodes, directions, s ) for s in start ]

    endState = int( reduce( lcm, moves ) )

    print( f"Part 2 answer: {endState}" )


def lcm( a, b ):
    return (abs( a ) / gcd( int(a), int(b) )) * abs( b )

# Finds the number of moves to get to a 'Z' ending for a single node
def findEnd( nodeLst, dirLst, node ):
    moves = 0
    foundEnd = False
