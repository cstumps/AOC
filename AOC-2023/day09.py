...
def main( argv ):
    ...
    print( f"Part 1 answer: {findSum( data )}" )
    ...
    print( f"Part 2 answer: {findSum( data )}" )
...
def solveSeq( seq ):
    # Compute the deltas all the way down
    s = [ np.array( seq ) ]

    while np.count_nonzero( s[ -1 ] ):
        s.append( np.diff( s[ -1 ] ) )

    # Backsolve for the next value in the initial sequence
    nextValSeq = [ 0 ]

    for i in range( len( s ) - 2, -1, -1 ):
        nextValSeq.append( nextValSeq[ -1 ] + s[ i ][ -1 ] )
