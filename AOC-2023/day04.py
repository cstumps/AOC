...
def main( argv ):
    ...
    # Start by determining number of matches for each card
    games = [ None ]

    for card in data:
        card = card.split( ':' )[ 1 ].split( '|' )

        winningNumbers = set( map( int, card[ 0 ].split() ) )
        myNumbers      = set( map( int, card[ 1 ].split() ) )

        games.append( len( winningNumbers.intersection( myNumbers ) ) )
    ...
    # This generates the correct answer but is a horrible solution.  Takes ~15 seconds to
    # run on my machine.  There must be a mathematical answer to this as well.

    q = queue.Queue()
    [ q.put( i ) for i in range( 1, len( games ) ) ]
    ...
    while not q.empty():
        ...
        if games[ card ]: # Only use games that were winners
            [ q.put( i ) for i in range( card + 1, card + games[ card ] + 1 ) ]

        scratchCards += 1 # Count the removed card in the total
