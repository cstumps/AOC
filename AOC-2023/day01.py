...
def main( argv ):
    ...
    for line in data:
        numbers = [ l for l in line if l.isdigit() ]
    ...
    for line in data:
        ...
        for k, v in mapTable:    
            fixedLine = fixedLine.replace( k, v )

        numbers = [ l for l in fixedLine if l.isdigit() ]
        sum += int( numbers[ 0 ] + numbers[ -1 ] )

        #print( f"Old line: {line.strip()}, new line: {fixedLine}, numbers: {numbers}, {numbers[0]+numbers[-1]}" )
