...
def main( argv ):
    ...
    for line in data:
        gameNum = int( line.split()[ 1 ][ 0:-1 ] )

        games[ gameNum ] = { 'red':   int( max( list( map( int, re.findall( '(\d+) red',   line ) ) ) ) ), 
                             'green': int( max( list( map( int, re.findall( '(\d+) green', line ) ) ) ) ), 
                             'blue':  int( max( list( map( int, re.findall( '(\d+) blue',  line ) ) ) ) ) }
