...
def main( argv ):
    ...
    for line in data:
        ...
        # Part 1 data
        value, strength = idHand( line[ 0 ] )
        hands.append( [ line[ 0 ], int( line[ 1 ] ), value, strength ] )

        # Part 2 data
        value, strength = idHand( line[ 0 ], wild=True )
        handsWild.append( [ line[ 0 ], int( line[ 1 ] ), value, strength ] )
    ...
    handsWild.sort( key=lambda x: x[ 3 ] )
    handsWild.sort( key=lambda x: x[ 2 ] )

    winnings = sum( [ (handsWild[ i ][ 1 ] * (i+1)) for i in range( len( handsWild ) ) ] )

    print( f"Part 2 answer: {winnings}" )

def idHand( hand, wild=False ):
    counts = Counter( hand ).values()
    jacks = hand.count( 'J' )

    if len( counts ) == 2:   # Four of a kind, full house
        if 4 in counts:
            if wild and jacks: # Elevate based on Joker
                val = HAND_FIVE
            else:
                val = HAND_FOUR
        else:
            if wild and jacks: 
                ...
                val = HAND_FULL
    elif len( counts ) == 3: # Three of a kind, two pair
        if 3 in counts:
            if wild and jacks:
                val = HAND_FOUR
            else:
            ...
            if wild and jacks:
                ...
                val = HAND_TWO
    elif len( counts ) == 1: # Five of a kind, one pair, no pairs
        val = HAND_FIVE
    elif len( counts ) == 4:
        if wild and jacks:
            val = HAND_THREE
    ...
    return val, computeStrength( hand, wild )

def computeStrength( hand, wild=False ):
    if wild:
        cards = 'J23456789TQKA'
    else:
        cards = '23456789TJQKA'
    ...
    for c in hand:
        strength = (strength << 4) + cards.index( c ) + 1
