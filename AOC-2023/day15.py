...
#     move any of the other lenses when you do this. If there aren't any lenses
...
def main( argv ):
    ...
    seq = sum( [ findHash( s ) for s in data.split( ',' ) ] )

    print( f"Part 1 answer: {seq}" )
    ...
    # Could have been done shorter without classes but it was fun and easy
    # to read.

    boxes = [ Box( i ) for i in range( 256 ) ]

    for step in data.split( ',' ):
        # Pull the label out of the data based on the type of instruction
        if step[ -1 ] == '-':                 # Remove from box
            label = step[ :-1 ]
            boxes[ findHash( label ) ].remove( label )
        else:                                 # Add to box
            label, focal = step.split( '=' )
            boxes[ findHash( label ) ].add( label, int( focal ) )

    # Compute and sum the power of all lenses acorss all boxes
    power = sum( [ b.power() for b in boxes ] )

    print( f"Part 2 answer: {power}" )
...
class Box:
    ...
    # Add a lens to a box (may or may not exist in box)
    def add( self, label, focal ):
        i = len( self.lenses )

        if label in self.lenses:
            i = self.lenses.index( label )
            self.lenses.remove( label )

        self.lenses.insert( i, Lens( label, focal ) )

    # Remove a lens from a box by label.
    def remove( self, label ):
        if label in self.lenses:
            self.lenses.remove( label )

    # Compute power of all lenses in box
    def power( self ):
        return sum( [ (1 + self.id) * (slot + 1) * lens.focal for slot, lens in enumerate( self.lenses ) ] )
