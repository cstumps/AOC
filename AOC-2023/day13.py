...
def main( argv ):
    ...
    for m in maps:
        ...
        # Save off the line where we saw the reflection (0 = no reflection on this axis)
        reflections.append( (r, c) ) 

        rows += r
        cols += c

    print( f"Part 1 answer: {(rows * 100) + cols}" )
    ...
    for m, ref in zip( maps, reflections ):
        rows += checkRowsSmudge( m, ref[ 0 ] )
        cols += checkRowsSmudge( m.T, ref[ 1 ] )

    print( f"Part 2 answer: {(rows * 100) + cols}" )


def checkRows( m ):
    for i in range( 0, len( m ) - 1 ):
        if (m[ i ] == m[ i + 1 ]).all():
            if not matchHalves( m, i ) or not i: 
                return i + 1
    
    return 0

def checkRowsSmudge( m, ref ):
    # For each row in map
    for i in range( len( m ) - 1 ):
        smudgeFound = False

        # If this was the same line as part 1, ignore it
        if ref and i == ref - 1:
            continue

        # Compare to the next row down and log how many differences there are
        diff = len( m[ i ] ) - (m[ i ] == m[ i + 1 ]).sum()

        # If there is 1 difference, we can consider this the smudge but don't allow for more smudges
        if diff == 1:
            smudgeFound = True

        # If there were 0 or 1 differences, proceed to check the rows
        if diff <= 1:
            diff = matchHalves( m, i )

            # If there were 0 differences and we fixed a smudge to get here, count it
            if not diff and smudgeFound:
                return i + 1

            # If there were 1 differences we can count it only if we didn't fix a smudge
            # while identifying the reflection line
            elif diff == 1 and not smudgeFound:
                return i + 1

            # Otherwise, don't count it

    return 0 # No line of symmetry found

# Returns number of differences between sub arrays
def matchHalves( m, i ):
    # Slice into two arrays and invert the second half.
    ar1 = np.flipud( m[ 0:i+1 ] )
    ar2 = m[ i+1: ]

    # Truncate down to the smallest size
    size = min( ar1.shape[ 0 ], ar2.shape[ 0 ] )

    # We truncate starting at the second element because we already know
    # the top two rows match.  This also allows us to detect one difference
    # in part two and then ignore it during the subsequent compare.
    ar1 = ar1[ 1:size ]
    ar2 = ar2[ 1:size ]

    return (ar1.shape[ 0 ] * ar1.shape[ 1 ]) - (ar1 == ar2).sum()
...
def readMap( data ):
    ...
    for line in data:
        ...
        m.append( list( map( lambda x: 0 if x == '.' else 1, line ) ) )
